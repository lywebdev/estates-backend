                            ...
                            <div class="card-body">
                                @if ($heating)
                                    <table id="estates-table" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                        @foreach ($heating as $row)
                                            <tr>
                                                <th>{{ $row->id }}</th>
                                                <th>{{ $row->name }}</th>
                                                <th class="table__btns">
                                                    <a href="{{ route('admin.estates.heating.edit', $row->id) }}" class="btn btn-outline-dark btn-flat mr-2">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <form action="{{ route('admin.estates.heating.destroy', $row->id) }}" method="post">
                                                        @csrf
                                                        @method('delete')
